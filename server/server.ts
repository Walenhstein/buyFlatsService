...
app.post('/api/post_order', async (req, res) => {
    try {
        ...
        if (!data.ok) return res.status(500).json({error: `Всё, заявочка чка. Не отправилась`})
        const clearData = await data.json();    
        return res.status(200).json(clearData);
    } catch (e) {
        if (e instanceof Error) return res.status(500).json({error: 'Ошибка внутреннего сервера'});
...
app.get('/api/:type/:id', async (req, res) => {
    try {
        ...
        if (!data.ok) return res.status(500).json({error: `Внешний API всёкэлэмэнэ, накрылсамэнэ`});
        const clearData = await data.json();
        return res.status(200).json(clearData);
    } catch (e) {
        if (e instanceof Error) return res.status(500).json({error: 'Ошибка внутреннего сервера'});
